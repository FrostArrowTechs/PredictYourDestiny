...
func (r zodiacComputeReq) toFortuneInput() fortune.Input {
	lang := r.Lang
	if lang == "" {
		lang = "zh-CN"
	}
...
// Compute returns zodiac fortune for the given birth year.
func (h *ZodiacHandler) Compute(c *gin.Context) {
	var req zodiacComputeReq
...
func (h *ZodiacHandler) computeChart(req zodiacComputeReq) (*fortune.Result, error) {
	eng, ok := fortune.Fortune(fortune.KindZodiac)
	if !ok {
		return nil, fmt.Errorf("zodiac engine unavailable")
	}
	return eng.Compute(req.toFortuneInput())
}

// Interpret computes zodiac fortune then asks AI for detailed reading.
func (h *ZodiacHandler) Interpret(c *gin.Context) {
	var req zodiacComputeReq
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	...
	spec, err := prompt.ZodiacBuild(req.toFortuneInput(), res)
	if err != nil {
...
func (h *ZodiacHandler) resolveModel(requested string, spec *fortune.PromptSpec) string {
...
func (h *ZodiacHandler) interpretSync(c *gin.Context, model string, msgs []ai.Message, opts ai.Options) {
	resp, err := h.Gateway.Chat(c.Request.Context(), model, msgs, opts)
	if err != nil {
		c.JSON(mapZodiacAIError(err), gin.H{"error": err.Error()})
		return
	}
...
func (h *ZodiacHandler) interpretStream(c *gin.Context, model string, msgs []ai.Message, opts ai.Options) {
...
func mapZodiacAIError(err error) int {
	switch {
