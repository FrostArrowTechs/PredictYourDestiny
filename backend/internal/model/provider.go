...
type AIProvider struct {
	...
	APIKey    string `gorm:"type:text"`                                                                      // versioned AES-GCM ciphertext
	Models    string `gorm:"type:text"`                                                                      // JSON array: [{"id":"gpt-4o","tier":"paid"},...]
	IsDefault bool   `gorm:"default:false;uniqueIndex:idx_ai_providers_one_default,where:is_default = true"` // only one provider can be default
